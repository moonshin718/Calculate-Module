...
namespace ConsoleApp1
{
    ...
    public class SplineInterpolator
    {
        ...
        public SplineInterpolator(IDictionary<double, double> nodes)
        {
            ...
            if (n > 2)
            {
                ...
                for (int i = 1; i <= n - 2; i++)
                {
                    ...
                    _c[i] = (_values[i + 1] - _values[i]) / _h[i + 1] - (_values[i] - _values[i - 1]) / _h[i];
                }

                SolveTridiag(sub, diag, sup, ref _a, n - 2, _c);
            }
        }
        ...
        /// </summary>
        private static void SolveTridiag(double[] sub, double[] diag, double[] sup, ref double[] b, int n,double[] values)
        {
            int i;
            for (i = 2; i <= n; i++)
            {
            ...
            for (i = n - 1; i >= 1; i--)
            {
                b[i] = (values[i] - sup[i] * values[i + 1]) / diag[i];
            }
        }
    }
}